...
public static class GameExtensions
{
    ...
    public static Vector2 SnappedTo(this Vector2 value, float step) =>
        new(Mathf.Snapped(value.X, step), Mathf.Snapped(value.Y, step));

    public static bool IsNullOrEmpty<T>(this ICollection<T> items) => items is null || items.Count == 0;

    public static T LastOr<T>(this IReadOnlyList<T> items, T fallback) =>
        items is null || items.Count == 0 ? fallback : items[items.Count - 1];

    public static int Clamped(this int value, int min, int max) => Mathf.Clamp(value, min, max);
}
