...
public static class Layers1
{
    ...
    public static Layer With(Layer mask, Layer layer) => mask | layer;

    public static Layer Without(Layer mask, Layer layer) => mask & ~layer;

    public static bool HasAny(Layer mask, Layer layers) => (mask & layers) != Layer.None;

    public static bool HasAll(Layer mask, Layer layers) => (mask & layers) == layers;
    ...
    public static int Count(Layer mask)
    {
        ...
        for (int bit = 1; bit <= (int)Layer.Pickup; bit <<= 1)
        {
            if (((int)mask & bit) != 0)
                count++;
        }
