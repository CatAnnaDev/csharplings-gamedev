...
public delegate int Combat(int attack, int defense);
...
public sealed class Emitter
{
    ...
    public int ListenerCount => Damaged is null ? 0 : Damaged.GetInvocationList().Length;
    ...
    public void HitSafely(int amount)
    {
        ...
        foreach (Delegate handler in Damaged.GetInvocationList())
        {
            try
            {
                ((Action<int>)handler)(amount);
            }
            catch (Exception)
            {
                Failures++;
