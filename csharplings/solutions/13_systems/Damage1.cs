...
public sealed class Fighter
{
    ...
    public float ResistanceTo(Element element) => _resistances.TryGetValue(element, out float ratio) ? ratio : 0f;
...
public static class Damage1
{
    ...
    public static int Compute(int raw, Element element, Fighter target, bool critical)
    {
        ...
        return Mathf.Max(Mathf.RoundToInt(value), 1);
    }
    ...
    public static int Apply(int raw, Element element, Fighter target, bool critical)
    {
        ...
        int dealt = Mathf.Min(Compute(raw, element, target, critical), target.Health);
