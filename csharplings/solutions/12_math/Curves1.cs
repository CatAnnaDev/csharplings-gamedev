...
public static class Curves1
{
    ...
    {
        Vector2 ab = a.Lerp(b, t);
        Vector2 bc = b.Lerp(c, t);
    ...
    {
        Vector2 abc = QuadraticBezier(a, b, c, t);
        Vector2 bcd = QuadraticBezier(b, c, d, t);
    ...
    public static Vector2 Arc(Vector2 start, Vector2 end, float height, float t)
    {
        ...
        return flat + Vector2.Up * (4f * height * t * (1f - t));
    }

    public static float Length(Func<float, Vector2> curve, int samples)
    {
        float total = 0f;
        Vector2 previous = curve(0f);

        for (int i = 1; i <= samples; i++)
        {
            Vector2 current = curve((float)i / samples);
